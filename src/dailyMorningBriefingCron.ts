...
function buildDailyMorningBriefingCronJob(nowMs: number) {
	return {
...
function comparableCronJobConfig(job: any) {
	return {
		id: job.id,
		agentId: job.agentId,
...
function cronJobConfigChanged(existing: any, nextJob: ReturnType<typeof buildDailyMorningBriefingCronJob>): boolean {
...
export async function ensureDailyMorningBriefingCron(runtime: PluginRuntime): Promise<void> {
	try {
		...
		const existingIndex = store.jobs.findIndex(job =>
			job.id === DAILY_MORNING_BRIEFING_CRON_ID || job.name === DAILY_MORNING_BRIEFING_CRON_ID
		);

		if (existingIndex >= 0) {
			const existing = store.jobs[existingIndex] as any;
			if (!cronJobConfigChanged(existing, nextJob)) return;
			...
			store.jobs.push(nextJob as any);
		}
