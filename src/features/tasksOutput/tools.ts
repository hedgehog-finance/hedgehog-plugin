...
import {
	GetTasksOutputParams,
	GetTasksOutputParamsSchema,
	QueryTasksOutputParams,
	QueryTasksOutputParamsSchema,
	RuntimeTool,
	SaveTasksOutputParams,
	SaveTasksOutputParamsSchema,
	TasksOutput
} from "./schema.js";
...
function requireSaveString(value: unknown, field: string): string {
	if (typeof value !== "string") {
...
function parseSaveTasksOutputParams(params: unknown): SaveTasksOutputParams {
	...
	const status = optionalString(raw.status, "status") as SaveTasksOutputParams["status"];
...
function parseGetTasksOutputParams(params: unknown): GetTasksOutputParams {
	const raw = (params || {}) as Record<string, unknown>;
...
function parseQueryTasksOutputParams(params: unknown): QueryTasksOutputParams {
	const raw = (params || {}) as Record<string, unknown>;
	return {
...
function selectTasksOutput(db: ReturnType<typeof getDB>, sessionId: string): TasksOutput | null {
	const row = db.prepare(`
		...
		FROM agent_sessions
		WHERE id = ?
	`).get(sessionId) as { taskId: string | null; workflowId: string | null; sessionId: string; output: string | null; createdAt: string; updatedAt: string } | undefined;
...
function syncSessionAndWorkStatus(db: ReturnType<typeof getDB>, sessionId: string, status: string, content: string) {
	const session = db.prepare("SELECT id, work_id, task_id FROM agent_sessions WHERE id = ?").get(sessionId) as { id: string; work_id?: string; task_id?: string } | undefined;
	if (session) {
...
export const tasksOutputTools: Record<string, RuntimeTool> = {
	save_task_output: {
		...
		async execute(params) {
			...
			const session = db.prepare("SELECT work_id, task_id, content FROM agent_sessions WHERE id = ?").get(args.sessionId) as { work_id?: string; task_id?: string; content?: string } | undefined;
			if (!session) {
	...
	query_task_outputs: {
		...
		async execute(params) {
			...
			const rows = db.prepare(`
				...
				LIMIT ? OFFSET ?
			`).all(...queryParams, args.pageSize, offset) as { taskId: string | null; workflowId: string | null; sessionId: string; output: string | null; createdAt: string; updatedAt: string }[];
